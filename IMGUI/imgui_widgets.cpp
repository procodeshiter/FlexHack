...
{
    static std::map<bool*, float> anim;
    anim[v] = lerp(anim[v], (float)*v, GImGui->IO.DeltaTime * 10);
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(6, 6);
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id))
    {
        IMGUI_TEST_ENGINE_ITEM_INFO(id, label, ImGui::GetItemStatusFlags() | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
        return false;
    }
    ...
    ImU32 check_col = ImColor(31,37,52);
    ImColor outlinecolor = collerp(ImColor(60,60,60), ImColor(107,90,222), anim[v]);
    ImColor fillcolor = collerp(check_col, ImColor(107,90,222), anim[v]);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    RenderNavHighlight(total_bb, id);

    const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
    const float pad2 = square_sz / 2;

    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, fillcolor, style.FrameRounding);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, outlinecolor, style.FrameRounding, 0, 2.5f * 1);
    AddRadialGradient(window->DrawList, check_bb.Min + ImVec2(pad2, pad2), 35 * 1, ImColor(107,90,222, (int)(anim[v] * 200.0f)), ImColor(107,90,222,0));

    bool mixed_value = (ImGui::GetItemFlags() & ImGuiItemFlags_MixedValue) != 0;
    if (mixed_value)
    {
    ...
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad) + ImVec2(square_sz / 2, square_sz / 2) * (1.0f - anim[v]), check_col, (square_sz - pad * 2.0f) * anim[v]);
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    ImGuiContext& g = *GImGui;
    if ((ImGui::GetItemFlags() & ImGuiItemFlags_ReadOnly) || (flags & ImGuiSliderFlags_ReadOnly))
        return false;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(4, 4), window->DC.CursorPos + ImVec2(4, 4) + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    if (!temp_input_is_active)
    {
        const bool input_requested_by_tabbing = temp_input_allowed && (window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_FocusedByTabbing) != 0;
        const bool clicked = (hovered && g.IO.MouseClicked[0]);
        if (input_requested_by_tabbing || clicked || g.NavActivateId == id || g.NavInputId == id)
        {
            ...
            FocusWindow(window);
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
            if (temp_input_allowed && (input_requested_by_tabbing || (clicked && g.IO.KeyCtrl) || g.NavInputId == id))
                temp_input_is_active = true;
        }
    }
    ...
    if (value_changed)
        ...
        anim1[p_data] = lerp(anim1[p_data], 0, GImGui->IO.DeltaTime * 10);


    ImVec2 linestart = ImVec2(frame_bb.Min.x, lerp(frame_bb.Min.y, frame_bb.Max.y, 0.5f));
    ImVec2 lineend = ImVec2(lerp(grab_bb.Min.x, grab_bb.Max.x, 0.5f), lerp(grab_bb.Min.y, grab_bb.Max.y, 0.5f));
    ImVec2 backgroundend = ImVec2(frame_bb.Max.x, lerp(frame_bb.Min.y, frame_bb.Max.y, 0.5f));
    ImVec2 linesize = ImVec2(0, 5);

    window->DrawList->AddRectFilled(linestart - linesize, backgroundend + linesize, ImColor(60,60,60), style.GrabRounding);
    window->DrawList->AddRectFilled(linestart - linesize, lineend + linesize, ImColor(107,90,222), style.GrabRounding);
    AddRadialGradient(window->DrawList, lineend, 30, ImColor(77,111,192, (int)(anim1[p_data] * 200.0f)), ImColor(77,111,192,0));
    window->DrawList->AddCircleFilled(lineend, 10 * (anim1[p_data] / 2.0f + 1.0f), ImColor(107,90,222));
